...
class Usuario extends CActiveRecord
{
	...
	public function rules()
	{
		...
		return array(
			array('id_tipo_usuario, email, login, senha', 'required'),
			array('id_tipo_usuario', 'numerical', 'integerOnly'=>true),
			array('nome', 'length', 'max'=>255),
			array('email', 'length', 'max'=>100),
			array('login, senha', 'length', 'max'=>32, 'min'=>32),
			// The following rule is used by search().
			// Please remove those attributes that should not be searched.
			array('id, nome, email, login', 'safe', 'on'=>'search'),
		);
	}
	...
	public function relations()
	{
		...
		return array(
			'materias' => array(self::HAS_MANY, 'Materia', 'id_usuario'),
			'tipo' => array(self::BELONGS_TO, 'TipoUsuario', 'id_tipo_usuario'),
		);
	}
